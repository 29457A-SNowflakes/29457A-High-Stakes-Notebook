...
#components.pro-con(
  ...
  cons: [
    - Limited lateral power: central strafing wheel in H Drive has less traction and pushing power compared to the Mecanum or X Drive, leading to weaker side-to-side movement
    ...
    - Programming complexity: is high but less so than X Drive
    - Space: central wheel means less space for other subsystems such as odometry pods
]
)

=== X Drive:
X Drive has traditionally been used by teams for extremely precise programming as it can provide an edge over more traditional drivetrains in terms of accurate movements in things like autonomous skills. However in competition settings, X Drive isn't very popular as its more difficult to control. X Drives use a compound gear ratio to get around the difficulties in making the wheels diagonal, this can mean that it becomes quite difficult to build and maintain in comparison with more traditional drivetrains. Trying to fit the brain, pneumatic tanks and other subsystems around this drivetrain can be difficult due to the odd motor placements.
#components.pro-con(
  pros: [
    - Faster: than both Mecanum and H-Drive, as it naturally provides more efficient power transfer for quick movements due to the 45-degree wheel orientation
    - Simpler: (compared to Mecanum Drive) to build and program, since it doesn’t require complex motor tuning or algorithms for strafing
    - Better diagonal movement: compared to other drivetrains
  ],
  cons: [
    - Lower torque: X Drive generally lacks traction and pushing force compared to Mecanum, which can cause it to struggle
    - Inefficient layout: the diagonal wheels takes up a lot of space, which limits available area for other mechanisms compared to more compact configurations like H-Drive
...
#figure([#image("./imgs/MecanumDrive.jpg", height: 100pt)], caption: [
  Model of a Mecanum Drive from the Servo magazine website about holonomic locomotion @ServoMagHolo
])
...
#components.pro-con(
 ...
 #components.pro-con(
  ...
  cons: [
    ...
    - Heavier: The greater weight will put more strain on motors, which leads to higher chance of motor burnout. However, this can be avoided by using hotswappable motors or cooling the motors with a fan.
  ]
 )
...
=== A Look at Gear Ratios
 #grid(
  ...
  rows: (auto),
  gutter: 10pt,
  [ #figure([#image("./imgs/gears.png", height: 100pt)], caption: [Image of a 2:3 gear coupling from wikipedia @GearTrainWiki])],
