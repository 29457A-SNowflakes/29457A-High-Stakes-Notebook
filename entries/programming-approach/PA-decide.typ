...
- We will prioritise uisng multiple files to aid with organisation
...
#admonition(type: "example", [
  Fun Fact: all of Meta's code (including Instagram, Facebook, Meta VR etc.) is all kept on the same Git codebase, meaning Zuckerburg's original Facebook code is available to _all_ meta developers
])
- Before commiting to the Git, we will make sure that all code follows the formatting rules
- If code changes have to be made in a rush -- for example during a competition -- commits to the Git will be flaged as bad code (often using the 💩 emoji#footnote([This is actually an industry standard flag for bad code])), and will be formatted at a later date.
