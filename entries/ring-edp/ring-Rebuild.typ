...
\
= Second stage 
The second stage is a hooks meck. The hooks come up from underneath hooking on to a ring, carrying it to the top and scoring it on a mobile goal. At least in theory that is how it would work.
...
The secound main issue we encountered when experimenting with our old intake was when more than 3 ring were scored on the mogo the hooks would get stuck and would not be able to complete a full rotation. This issue would be truly catastrophic unresolved as it would limit your possible points scored to 5/8 or require us to make the intake go backwards and forwards instead of cycling properly.
#align(center)[
