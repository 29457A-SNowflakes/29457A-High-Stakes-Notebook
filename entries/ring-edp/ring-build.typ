...
- We used high strength shafts since they are incredibly rigid and help mitigate where friction could occur, since less bending occurs, the flex wheels, flex wheels already have an insert to convert them from hexagons a second to convert to low strength shaft is one we want to avoid. 
