...
= Final decision
We decided to go with a two bar mechanism, which picks up rings at the apex of the hooks mechanism, because we believe that it is the most consistent solution across the board for our robot. Unlike redirects, there is much less need to tune to get it functional, which is valuable time saved. In addition, this solution doesn't clog up the hook and stop you from intaking whilst the ring redirects to a scoring mech. A piston redirect, doesn't suffer from that issue but it has limited actuations, which may become a problem in match scenarios and secondly isn't compatable with a hook based ring mech, which would require us to *completely scrap* that entire mechanism and tune for that instead, which is something we as a team given the viability of other solutions. Whilst, fish mech is probably the simplest to implement, our analysis from watching matches, which we have watched has shown that it is many ways inferior to a normal two bar mechanism since it is much slower, requires more precision from the driver and also much easier to defend against.